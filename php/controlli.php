...
function controllaADomicilio($aDomicilio, &$messaggiForm){
  ...
  if(!isset($aDomicilio) || !( $aDomicilio == "1" || $aDomicilio== "0") ){
		$messaggiForm .= "<p class='phpError'> il campo \"a domicilio\\in studio \" deve essere specificato </p>";
    return NULL; 
	}
  else{
    return $aDomicilio;
...
function controllaEmail(&$email,&$messaggiForm){
  if(isset($email)){
		if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
			$messaggiForm .= "<p class='phpError'>L'email inserito non è in un formato valido </p>";
		}else{
			return $email;
		}
	}else{
		$messaggiForm .= "<p class='phpError'> il campo \"email\" deve essere specificato </p>";
	}
  return "";
}
...
function controllaCel(&$cel, &$messaggiForm){
  if(isset($cel)){
    ...
    if(preg_match("/^([+]d{2})?\s*\d{3}\s*\d{3}\s*\d{4}$/", $cel)){
        ...
        $messaggiForm .= "<p class='phpError'>il formato del campo cellulare deve essere 111 222 3333 oppure +11 222 333 4444</p>";
    }
}
else{
    $messaggiForm .= "<p class='phpError'>inserire il numero di cellulare</p>";
}
return "";
}
...
function controllaNome(&$nome, &$messaggiForm){
  if(isset($nome)){
    ...
    if (preg_match("/^[a-zA-Z' ]*$/",$nome)){
        ...
        $messaggiForm .= "<p class='phpError'>Il nome deve essere composto solo da lettere</p>";
    }
}else{
    $messaggiForm .= "<p class='phpError'>Il nome deve essere specificato </p>";
}
return "";
}
  ...
  controllaInput($indirizzo);
    if (preg_match("/^[a-zA-Z0-9' ]*$/",$indirizzo)){
        return $indirizzo;
          ...
          $messaggiForm .= "<p class='phpError'>L' indirizzo deve essere composto solo da lettere e numeri</p>";
      }
  }else{
    $messaggiForm .= "<p class='phpError'>L' indirizzo deve essere specificato in tutte le prenotazioni a domicilio</p>";
  }
  return "";
}
?>